...
async def explore_news_site_step(
    state: NewsSiteExplorationState,
    agent: Agent[ScrapeInformation, NewsSiteExplorationDependencies],
) -> ScrapeInformation:
    attempt_number = 1
    prompt = build_feed_exploration_prompt(state.input_data, attempt_number)
    exploration_result = await _run_exploration_with_agent(agent=agent, prompt=prompt)
    state.latest_result = exploration_result
    state.sample_article_urls = []
    return exploration_result


async def extract_sample_articles_step(
    state: NewsSiteExplorationState,
    agent: Agent[ScrapeInformation, NewsSiteExplorationDependencies],
) -> ScrapeInformation:
    _ = agent
    ...
    unique_sorted_article_urls = sorted(set(article_urls))
    max_sample_size = min(len(unique_sorted_article_urls), state.sample_articles_count)
    state.sample_article_urls = await _select_structured_article_urls(
        article_urls=unique_sorted_article_urls,
        sample_size=max_sample_size,
    )
    return latest_result


async def explore_articles_step(
    state: NewsSiteExplorationState,
    agent: Agent[ScrapeInformation, NewsSiteExplorationDependencies],
) -> ScrapeInformation:
    partial_result = state.latest_result
    ...
    attempt_number = state.article_refinement_attempts + 1
    prompt = build_article_exploration_prompt(
        input_data=state.input_data,
        attempt_number=attempt_number,
        partial_result=partial_result,
        sample_article_urls=sample_article_urls,
        previous_validation_error=state.last_validation_error,
    )
    exploration_result = await _run_exploration_with_agent(agent=agent, prompt=prompt)
    merged_result = _merge_article_level_selectors(
        base_result=partial_result,
        refinement_result=exploration_result,
    )
    normalized_result = await _normalize_article_level_selectors(
        result=merged_result,
        sample_article_urls=sample_article_urls,
    )
    state.article_refinement_attempts = attempt_number
    state.latest_result = normalized_result
    return normalized_result


async def _run_exploration_with_agent(
    *,
    agent: Agent[ScrapeInformation, NewsSiteExplorationDependencies],
    prompt: str,
) -> ScrapeInformation:
    ...
    for local_attempt in range(1, max_agent_attempts + 1):
        ...
        try:
            run_result = await agent.run(strict_prompt)
...
async def _normalize_article_level_selectors(
    *,
    ...
    recovered_main_selector = await _recover_stable_main_selector(sample_article_urls)
...
def _should_fallback_main_selector(selector: str) -> bool:
    ...
    starts_with_generic_article = selector in {"article", "body article", "h1 + div"}
    is_generic_chain = _looks_like_generic_chain(selector)
...
def _should_fallback_author_selector(selector: str) -> bool:
    is_empty_selector = selector == ""
    if is_empty_selector:
        return False
...
def _looks_like_generic_chain(selector: str) -> bool:
...
async def _recover_stable_main_selector(sample_article_urls: list[str]) -> str | None:
    candidate_selectors = [
    ...
    for candidate_selector in candidate_selectors:
        is_valid_for_all = all(
            _is_main_selector_candidate_valid(
                soup=soup,
...
def _is_main_selector_candidate_valid(*, soup: BeautifulSoup, selector: str) -> bool:
    ...
    *,
    ...
    async with httpx.AsyncClient(
        ...
        for article_url in article_urls:
            ...
            is_structured = await _is_structured_article_page(
                client=client,
            ...
            candidate_selection = [*selected_urls, article_url]
            shared_selector = await _recover_stable_main_selector(candidate_selection)
...
async def _is_structured_article_page(
    *,
    ...
    has_usable_main_candidate = _has_usable_main_candidate(soup)
...
def _has_usable_main_candidate(soup: BeautifulSoup) -> bool:
    candidate_selectors = [
        "article[itemprop='articleBody']",
        "[itemprop='articleBody']",
        "main article",

...
async def test_audio_downloader_retries_and_preserves_order(tmp_path: Path) -> None:
    links = [
        MediaDownloadableLink(url="https://example.com/2.aac", sequence_number=2),
        MediaDownloadableLink(url="https://example.com/1.aac", sequence_number=1),
    ]
    responses = {
        "https://example.com/1.aac": [(429, []), (200, [b"one"])],
        "https://example.com/2.aac": [(200, [b"two"])],
    }
    ...
    downloaded = await downloader.download_audio()

    assert downloaded.stream_type == SupportedStreamTypes.DASH
    assert [chunk.source_url for chunk in downloaded.chunks] == [
        "https://example.com/1.aac",
        "https://example.com/2.aac",
    ]
    assert downloaded.chunks[0].file_path.read_bytes() == b"one"
    assert downloaded.chunks[1].file_path.read_bytes() == b"two"


async def test_audio_downloader_cleans_up_on_failure(tmp_path: Path) -> None:
    responses = {"https://example.com/fail.aac": [(500, [])]}
    downloader = AudioDownloader(
        path_to_download=tmp_path,
        chunks_data=[
            MediaDownloadableLink(
                url="https://example.com/fail.aac",
                sequence_number=1,
            )
        ],
        source_url="https://example.com/audio",
